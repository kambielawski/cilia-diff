...
def main():
    exp_name = args.exp_file.split('/')[-1].split('.')[0]
    os.makedirs(f'./experiments/{exp_name}', exist_ok=True)
    os.system(f'cp {args.exp_file} ./experiments/{exp_name}')
    
    for arm in exp_arms:
        # Create experiments directory if it doesn't already exist
        os.makedirs(f'./experiments/{exp_name}/{arm}', exist_ok=True)
        ...
        dc = DiffControl(savedata_folder=f'./experiments/{exp_name}/{arm}', experiment_parameters=experiment_parameters)
        dc.init(rbt)
